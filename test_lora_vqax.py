...
def filter_and_get_scores(test_data, full_predictions, exp_predictions, annFile_path, resFile_path, scoresFile_path):
    ...
    print("Accuracy: {:.3f}".format(len(correct_keys) / len(pred_answers.keys())))
    accuracy_value = "Accuracy: {:.3f}".format(len(correct_keys) / len(pred_answers.keys()))
...
img_global_path = 'data/'           # ✅ fixed: was hardcoded to another user's machine
model_path = 'model/'
img_size = 224
temperature = 1
top_k = 0
...
# ── Load Data ─────────────────────────────────────────────────────────────────
data = json.load(open('data/vqaX_test.json', 'r'))

# ── Image Encoder ─────────────────────────────────────────────────────────────
image_encoder = CLIPEncoder(device)

# ── Tokenizer (initial load for dataset, from local) ─────────────────────────
tokenizer = GPT2Tokenizer.from_pretrained(
    model_path + 'model_tokenizer',
    local_files_only=True   # ✅ fixed: load from local folder, no internet call
)
...
# ── Annotation File ───────────────────────────────────────────────────────────
annFile_path = ann_main_path + 'vqax_test_annot_exp.json'
...
print("Valid samples:", len(filtered_dataset))

# ✅ fixed: use filtered_dataset in DataLoader, not raw dataset_class (which may contain None)
...
model = GPT2LMHeadModel.from_pretrained(
    ...
    local_files_only=True   # ✅ fixed: was hitting HuggingFace Hub
).to(device)
...
# ── Reload Tokenizer with Special Tokens (from local) ────────────────────────
tokenizer = GPT2Tokenizer.from_pretrained(
    model_path + 'model_tokenizer',
    local_files_only=True   # ✅ fixed: was calling from_pretrained('gpt2') hitting internet
)
orig_num_tokens = len(tokenizer)  # ✅ fixed: .encoder removed in newer transformers, use len(tokenizer)
...
model.load_state_dict(
    torch.load(model_path + 'model_lora.bin', map_location=device)  # ✅ use model_path variable
)
model.eval()

# ── Run Evaluation ────────────────────────────────────────────────────────────
loraResults = results_main_path + 'lora_vqax_results.json'
scoresResults = results_main_path + 'lora_vqax_evalMetrics.json'
