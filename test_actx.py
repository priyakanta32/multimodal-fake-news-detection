...
class TestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # ✅ filter missing images instead of crashing
        if os.path.exists(img_full_path):
            ...
            print("Image not found:", img_full_path)
            return None
...
def get_evalMetrics(preds, full_predictions, test_data, annotation_file, resFile_path, scoresFile_path):
    ...
    cocoEval.params['image_id'] = cocoRes.getImgIds()  # ✅ avoid ID mismatch
    cocoEval.evaluate()
...
# ── Load Data ─────────────────────────────────────────────────────────────────
test_dataset_file = 'data/actX_test.json'
data = json.load(open(test_dataset_file, 'r'))
...
# ── Paths ─────────────────────────────────────────────────────────────────────
annotation_file = 'cococaption/annotations/actX_test_annot_exp.json'
resFile_path    = results_main_path + 'actx_results.json'
scoresFile      = results_main_path + 'actx_evalMetrics.json'

# ── Run Evaluation ────────────────────────────────────────────────────────────
results_full, results_exp = sample_sequences(model, tokenizer, test_loader, 50)
get_evalMetrics(results_exp, results_full, data, annotation_file, resFile_path, scoresFile)